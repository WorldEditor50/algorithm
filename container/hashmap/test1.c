...
void test_HashMap()
{
	...
	/* register object */
	iRet = HashMap_RegisterObject(pstMap, freeValue, NULL, NULL, NULL);
	TEST_ASSERT(iRet != HASHMAP_OK, "failed to register");
	/* insert */
	...
	/* resize */
	printf("%s\n", "----------resize------------------");
	iRet = HashMap_Resize(pstMap, 20);
	TEST_ASSERT(iRet != HASHMAP_OK, "failed to resize");
	iRet = HashMap_Traverse(pstMap, printHashNode, NULL);
	TEST_ASSERT(iRet != HASHMAP_OK, "failed to traverse");
	/* destroy hashmap */
