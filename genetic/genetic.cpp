...
float Genetic::run(char optType, int iterate)
{
...
void Genetic::eliminate(char optType)
{
    /* sort */
...
{
    this->create(crossRate, mutateRate, maxGroupSize, maxCodeLen);
}
...
{
    /* exchange quater of the genes */
    int crossNum = rand() % code.size() / 4;
